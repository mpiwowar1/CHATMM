...
function normalizeMessage(
  ...
  return {
    ...
    text: decodeCiphertext(response.ciphertext),
    timestamp: new Date(response.timestamp),
    isOwn: String(response.senderId) === currentUserId,
  }
}

function buildStompUrl() {
  const apiBase = baseip.replace(/\/api\/v1$/, "")
...
function createStompClient(
  ...
  const socket = new WebSocket(buildStompUrl())
  ...
  const sendFrame = (
    ...
    const frame = frameStomp(command, headers, body)
  ...
  socket.addEventListener("message", (event) => {
    const frames = parseStompFrame(event.data)
  ...
  const subscribe = (destination: string) => {
    ...
    subscriptions.set(subscriptionId, destination)
    sendFrame("SUBSCRIBE", {
      id: subscriptionId,
      destination,
    })
        ...
        <div className="flex flex-1 flex-col items-center justify-center p-8 text-center">
          <div className="rounded-2xl border bg-card p-8 shadow-lg">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              Create your first conversation using the plus button in the
              sidebar.
            </p>
          </div>
        </div>
